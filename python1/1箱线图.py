...
df_stress=pd.read_csv('stress_cable.csv')
df=pd.concat([df_dis, df_stress], axis=1)
...
#%%
grouped = df.groupby(["A14", "A23"])
df3 = pd.DataFrame({col:vals['23y'] for col,vals in grouped})
meds = df3.median()
meds.sort_values(ascending=True,inplace=True)
df3 = df3[meds.index]
df3.boxplot()
...
#温度对最大stress的影响
#%%
df.boxplot(column = '31', by = ['A14','A23'])
#plt.ylim([-35, -25])#只调y轴
plt.show()

#%%
grouped = df.groupby(["A14", "A23"])
df4 = pd.DataFrame({col:vals['31'] for col,vals in grouped})
meds = df4.median()
meds.sort_values(ascending=True,inplace=True)
df4 = df4[meds.index]
df4.boxplot()
